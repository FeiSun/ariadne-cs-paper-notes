...
The saved HTML must contain an inspectable paper annotation workspace:
...
- Never use the visible labels `位置`, `原句/片段`, or `证据/验证` inside paper-reader margin cards for sentence, paragraph, section, or paper anchors.
- Show `核查依据` in a margin card only when it adds manuscript-level evidence the reader could not infer from the anchor itself, such as a table recomputation, PDF page/layout observation, visible citation/reference issue, visible checklist/submission risk, or human/judge audit detail. Hidden source hygiene, hidden reference metadata, and macro-only findings must stay in structured artifacts with `render_visibility: artifact_only` unless they visibly affect the rendered paper. Do not show mechanical provenance such as `data-sentence-id`, `data-paragraph-id`, `source-derived paper-reader`, or `generated by render_paper_html.py`.
