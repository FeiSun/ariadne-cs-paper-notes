...
- The annotation card explains `问题是什么`, `为什么有问题`, one atomic `违反原则`, `严重度理由`, `关联问题` when relevant, and `自改问题`. Avoid showing command-style `下一稿任务` in margin cards unless the user explicitly requested a revision plan.
