...
- Prose critique rule refs under `references/rules/`: use `core_principles.md`, `prose_phase_a_rules.md`, `prose_phase_b_rules.md`, `method_rules.md`, `experiment_rules.md`, `related_work_rules.md`, and `prose_style_rules.md` when running or inspecting Prose Phase A/B. `scripts/build_prose_phase_packet.py` exposes these as executable `rule_refs`; `scripts/run_prose_agent.py` resolves them into `ARIADNE_PROMPT_FILE` before invoking the Prose Agent.
- Specialist maintenance docs under `references/rules/`: `layout_pdf_rules.md`, `numeric_rules.md`, `figure_table_caption_rules.md`, `reference_rules.md`, `source_hygiene_rules.md`, `symbol_rules.md`, and `polish_rules.md` document judgment boundaries only. They are not loaded into default specialist packets; deterministic `check_*.py` scripts remain the detection source.
- `references/review_lenses.md`: legacy compact lens reference retained for compatibility while the role-specific `references/rules/` files are the preferred Prose rule source.
...
5. Use the two-phase Prose Review Agent for full-paper critique:
   - Phase A reads the full review units plus executable Prose `rule_refs` (`core`, Phase A, method, experiment, related-work, prose-style) and runs cold skim, linear deep read, paragraph decisions, and section reflections. It writes `prose_issues.jsonl`, `paragraph_decisions.jsonl`, `section_reflections.json`, `cold_skim_frame.json`, and `claim_candidates.json` incrementally in article order.
   - Phase B reads only compact derived inputs (`phase_b_context.json`, curated specialist `*_issues.json`, and claim candidates) plus executable synthesis `rule_refs` (`core`, Phase B, method, experiment, related-work) to produce `argument_map.json`, `claims.json`, `salvageable_core.json`, and `whole_paper_findings.jsonl`.
   - `scripts/run_prose_agent.py --bundle <bundle> --agent-cmd "<external agent command>"` can run the Phase A/B loop through a pluggable agent. The command receives `ARIADNE_PROMPT_PACKET`, `ARIADNE_PROMPT_FILE`, `ARIADNE_BUNDLE`, and `ARIADNE_ISSUE_ARTIFACTS`; it must read the prompt file when invoking a model and write the JSON/JSONL targets named in the packet. Use `--dry-run` to validate packets and rendered prompts without calling the agent. The runner detects `no_progress` if an agent returns successfully without advancing resume/write-target counts.
   - `rule_refs` are references in the packet, not embedded rule text. `run_prose_agent.py` renders `ARIADNE_PROMPT_FILE` with resolved rule text before invoking `--agent-cmd`; standalone packet adapters should use `scripts/run_agent_command.py --packet <packet> --command "<provider command>"` or equivalent rule resolution.
...
6. Keep prose judgment and sidecar artifact checks separate. Launch specialist subagents conditionally for layout, numeric/table, references, symbol/term consistency, source hygiene/anonymity, figure/caption, and polish only when the corresponding raw audit or manuscript signal exists. Each specialist reads its own raw audit and writes a compact `*_issues.json`; the orchestrator reads issue artifacts, not raw audits. Skipped specialists should emit an empty issue stub when downstream tooling expects the artifact.
   ...
   - Optional LLM specialist refinement can run after deterministic reducers with `scripts/run_specialist_agent.py --issues-dir <bundle>/issue_artifacts --agent-cmd "<external specialist command>"`. The specialist command receives only curated issue artifacts by default and no specialist `rule_refs`; specialist rule docs are maintenance-only unless a future workflow explicitly upgrades a domain to executable refinement rules. Do not pass raw audits into the orchestrator.
...
Runtime files are `SKILL.md`, `references/*.md`, `references/rules/*.md`, `scripts/extract_paper_text.py`, and `scripts/build_paper_pdf.py`. `tests/` is for development/regression checks. `agents/openai.yaml` is Codex UI metadata; keep it aligned with this skill, but do not rely on it for review logic.
